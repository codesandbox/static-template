__set_movable = (obj, _mv, s = "") => {
  if (_mv) {
    document.getElementById(obj.id + "-titlebar" + s).onmousedown = function (
      e
    ) {
      pointerX = e.clientX;
      pointerY = e.clientY;
    ...
    document.getElementById(obj.id + "-titlebar" + s).ontouchstart = function (
      e
    ) {
    ...
    document.getElementById(obj.id + "-titlebar").ontouchstart = function (
      e
    ) {};
  }
};
...
function AllowMovable(obj) {
  ...
  if (document.getElementById(obj.id + "-titlebar2") != null) {
    // For special titlebars
    [2, 3, 4].forEach((s) => {
      __set_movable(obj, true, s);
    });
  }
}
...
function window_maximize(w) {
  window_infos[w] = [
    document.getElementById(w).style.top,
    document.getElementById(w).style.left,
    document.getElementById(w).style.width,
    document.getElementById(w).style.height
  ];
  ...
  DisableMovable(w);
  document.getElementById(w + "-maximize").src =
    "icons/Suru/Suru/scalable/ui/window-restore-symbolic.svg";
  document.getElementById(w + "-maximize").onclick = function (e) {
    window_restore(w);
  };
...
function window_restore(w) {
  ...
  document.getElementById(w + "-maximize").src =
    "icons/Suru/Suru/scalable/ui/window-maximize-symbolic.svg";
  document.getElementById(w + "-maximize").onclick = function (e) {
    window_maximize(w);
  };
...
function window_create(
  ...
  document
    ...
    .insertAdjacentHTML(
      ...
      '<div id="' +
        name +
        '" onmousedown="changePanelActiveAppIcon(\'' +
        icon_grayscale +
        "','" +
        title +
        "'," +
      ...
      <div id="' +
        name +
        '-titlebar" class="window-titlebar">\
          <div class="window-titlebar-title">' +
        title +
        '</div>\
          <div class="window-titlebar-control">\
              <img class="window-titlebar-control-icon" onclick="window_minimize(\'' +
        name +
        '\');" draggable="false" src="icons/Suru/Suru/scalable/ui/window-minimize-symbolic.svg"/>&nbsp;&nbsp;\
              <img id="' +
        name +
        '-maximize" class="window-titlebar-control-icon" onclick="window_maximize(\'' +
        name +
        '\');" draggable="false" src="icons/Suru/Suru/scalable/ui/window-maximize-symbolic.svg"/>&nbsp;&nbsp;\
              <img class="window-titlebar-control-icon" onclick="window_delete(\'' +
        name +
        '\');" draggable="false" src="icons/Suru/Suru/scalable/ui/window-close-symbolic.svg"/>&nbsp;&nbsp;\
          </div>\
      </div>\
      <iframe id="' +
        name +
        '-content" allowfullscreen class="window-content" src="' +
        url +
        '"></iframe>\
  </div>'
    );
  ...
  document.getElementById("dock").innerHTML +=
    '<div id="' +
    name +
    '-dock" class="dock-app-container">\
      <div class="dock-app-active"></div>\
      <img class="dock-app" onclick="window_minimize(\'' +
    name +
    '\');" draggable="false" src="' +
    icon +
    '"/>\
...
function window_create_special(
  name,
  url,
  title,
  icon,
  ...
  document
    ...
    .insertAdjacentHTML(
      ...
      <div id="' +
        name +
        '-titlebar" class="window-titlebar-special">\
          <div class="window-titlebar-control">\
              <img class="window-titlebar-control-icon" onclick="window_minimize(\'' +
        name +
        '\');" draggable="false" src="icons/Suru/Suru/scalable/ui/window-minimize-symbolic.svg"/>&nbsp;&nbsp;\
      ...
      <div id="' +
        name +
        '-titlebar2" class="window-titlebar-special-2"></div>\
      <div id="' +
        name +
        '-titlebar3" class="window-titlebar-special-3"></div>\
      <div id="' +
        name +
        '-titlebar4" class="window-titlebar-special-4"></div>\
      <iframe id="' +
        name +
        '-content" allowfullscreen class="window-content-special" src="' +
        url +
        '"></iframe>\
  </div>'
    );
