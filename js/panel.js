  ...
  (async () => {
    ...
    updateBatteryIcon = () => {
      if (deviceBattery.charging) {
        batteryIcon.src =
          "icons/Suru/Suru/scalable/status/battery-level-" +
          Math.round(deviceBattery.level * 10) * 10 +
          "-charging-symbolic.svg";
        document
          .querySelector(".status-about")
          .contentDocument.querySelector("#battery").src =
          "icons/Suru/Suru/scalable/status/battery-level-" +
          Math.round(deviceBattery.level * 10) * 10 +
          "-charging-symbolic.svg";
        if (deviceBattery.level == 1) {
          document
            .querySelector(".status-about")
            .contentDocument.querySelector("#battery-info").innerText = "Full";
        } else {
          if (deviceBattery.chargingTime != Infinity) {
            document
              .querySelector(".status-about")
              .contentDocument.querySelector("#battery-info").innerText =
              Math.floor(((1 - deviceBattery.level) * 100 * 60 * 3) / 3600) +
              ":" +
              Math.floor(
                (((1 - deviceBattery.level) * 100 * 60 * 3) % 3600) / 60
              ) +
              " Until Full (" +
              deviceBattery.level * 100 +
              " %)";
          }
        }
      } else {
        ...
        if (deviceBattery.dischargingTime != Infinity) {
          document
            .querySelector(".status-about")
            .contentDocument.querySelector("#battery-info").innerText =
            Math.floor(deviceBattery.dischargingTime / 3600) +
            ":" +
            Math.floor((deviceBattery.dischargingTime % 3600) / 60) +
            " Remaining (" +
            deviceBattery.level * 100 +
            " %)";
        } else {
          document
            .querySelector(".status-about")
            .contentDocument.querySelector("#battery-info").innerText =
            Math.floor((deviceBattery.level * 100 * 60 * 3) / 3600) +
            ":" +
            Math.floor(((deviceBattery.level * 100 * 60 * 3) % 3600) / 60) +
            " Remaining (" +
            deviceBattery.level * 100 +
            " %)";
        }
...
window.addEventListener("online", () => {
  wifiIcon.src =
    "icons/Suru/Suru/scalable/status/network-wireless-connected-symbolic.svg";
  document
    .querySelector(".status-about")
    .contentDocument.querySelector("#wifi").src =
    "icons/Suru/Suru/scalable/status/network-wireless-connected-symbolic.svg";
  document
    .querySelector(".status-about")
    .contentDocument.querySelector("#wifi-info").innerText = "Wi-Fi";
  document.querySelector(".panel-status").className = document
    .querySelector(".panel-status")
    .className.replace(" panel-status-no-wifi", "");
});

function UpdatePanel() {
  now = new Date();
  document.getElementsByClassName("panel-notification")[0].innerHTML =
    now.getDate() +
    " " +
    monthShortNames[now.getMonth()] +
    " " +
    now.toString().match("..:..")[0];
}
...
function changePanelActiveAppIcon(icon, name, brightness = 1) {
  ...
  document.querySelector(".panel-active-app-icon").style.filter =
    "brightness(" + brightness + ") grayscale(1)";
  setTimeout(() => {
    document.querySelector(".panel-active-app").style.width =
      Math.abs(
        document
          .getElementsByClassName("panel-active-app-icon")[0]
          .getBoundingClientRect().left -
          document
            .getElementsByClassName("panel-active-app-name")[0]
