...
setTimeout(async function() {
    ...
    setCSSVar('--themeColor', JSON.parse(storage.betterPhotop).themeColor ? JSON.parse(storage.betterPhotop).themeColor:'#5AB7FA')
    ...
    showPopUp("All ready!", "Your configurations are ready! (theme and keybinds)", [["Close", "gray", null]]);

    var rateLimit = 0;
    socket.subscribe({Type: "Typing"}, function (Data, Config) {
        if (Data.Username != Username) {
            if (Data.Typing) {
                document.getElementById(Data.PostID + "postChatChatting").innerHTML = "&bull; " + Data.Username + " is typing...";
            } else {
                document.getElementById(Data.PostID + "postChatChatting").innerHTML = "&bull; 0 Chatting";
            }
        }
    });
            ...
            document.activeElement.addEventListener("input", function (e) {
                if (Date.now() > rateLimit) {
                    if (document.activeElement.innerHTML.length > 0) {
                        socket.publish({Type: "Typing"}, {PostID: document.activeElement.id.replace("TextBox", ""), Username: Username, Typing: true});
                        rateLimit = Date.now() + 300;
                    } else {
                        socket.publish({Type: "Typing"}, {PostID: document.activeElement.id.replace("TextBox", ""), Username: Username, Typing: false});
                        rateLimit = Date.now() + 300;
                    }
                }
            });
