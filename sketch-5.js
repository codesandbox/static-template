...
let body = 
        ...
        for (let i = 0; i < poses.length; i++) {
            ...
            for (let j = 0; j < pose.keypoints.length; j++) {
                ...
                if (keypoint.score > 0.2) { //0.2
                    ...
                    //リアルタイムスケルトンの座標格納配列[]
                    json += [body[j]+","+parseInt(pose.keypoints[j].position.x)+","+parseInt(pose.keypoints[j].position.y)+","+"\n"];
                }
            }
        }
    };
    ...
    p.blob = function(){//CSVファイルにしてダウンロードできるように
        const fr = 10; 
        p.frameRate(fr);
...
let text;
    ...
    p.draw = function() {
        ...
        form.myfile.addEventListener('change', function(e) { //ファイルが読み込まれた時の処理
            ...
            reader.onload = function(ev){
                ...
                for (k = 0; k < results2.length; k++) {  
                 ...
                 /*const fr = 100; 
                 p.frameRate(fr);*/
                 for (m = 0; m < 16; m++) {//骨格点の表示(for文でコードの短縮化)
                     p.ellipse(parseInt(json2[m+7*l][1]),parseInt(json2[m+7*l][2]), 10, 10);
                 }
   ...
   　　　　　      document.getElementById("PassageArea00_x").innerHTML = match00_x; //表示更新
            ...
            　　
                 match0 = Math.abs((Number(match0_xx.textContent)) - match00_x)+difference;//座標一致度計算
                 match1 = Math.abs((Number(match1_xx.textContent)) - match01_x)+difference;//座標一致度計算
                 match2 = Math.abs((Number(match2_xx.textContent)) - match02_x)+difference;//座標一致度計算
                 match3 = Math.abs((Number(match3_xx.textContent)) - match03_x)+difference;//座標一致度計算
                 match4 = Math.abs((Number(match4_xx.textContent)) - match04_x)+difference;//座標一致度計算
                 ...
                 /*$(function () {
                    ...
                    $(".button1").on("click", showdata);
                    async function showdata() {
                        data = await loadSheets(sheetId, range);
                        text = data.values[1][1]; //[][]縦列・横列
                        //speak(text);
                    }
                 });*/
