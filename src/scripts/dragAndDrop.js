...
function checkIsHoveringSubActionsTable(evt) {
	...
	var isHoveringOverSubActionTable =
		...
		/(list:[0-9]+_subs)/.test(evt.target.className);
	...
	isDropAreaEnabled =
		subActionTableElement &&
		subActionTableElement.dataset &&
		boolify(subActionTableElement.dataset.dropTarget);
...
function setHoverTargets(
	...
	if (
		...
		isIdMatching = newHoverTarget.id === subActionTable.tableElement.id;
	} else {
...
function isHoveringOverEnabledDropArea(e) {
	...
	var isHoveringOverRow =
		...
		!/_subs:/.test(e.target.parentNode.id);
	/**
...
function resetFunction(childId) {
	...
	cursor_wait();
	oamSubmitForm("mainForm", "drag", null, [["childId", childId]]);
}
